...
class AdapterBills(
    ...
    override fun getMineBills(userId: Long, isCheck: Boolean) {
        ...
        filteredData.clear()
        if (isCheck) {
            val tempDataBills = mutableListOf<BillsResponse.TableGroup.Table.Bill>()
            val tempDataTable = mutableListOf<BillsResponse.TableGroup.Table>()
            data.forEach { tableGroup ->
                if (!tableGroup.tables.isNullOrEmpty()) {
                    tableGroup.tables.forEach { table ->
                        if (!table.bills.isNullOrEmpty()) {
                            tempDataBills.addAll(table.bills.filter { it.createdByUserId == userId })
                        }
                    }
                }
            }
            val tableId = tempDataBills.map {
                it.tableId
            }
            data.forEach {
                if (!it.tables.isNullOrEmpty()) {
                    tempDataTable.addAll(it.tables.filter { table -> table.tableId in tableId })
                }
            }
            val tableGroupId = tempDataTable.map {
                it.tableGroupId
            }
            filteredData.addAll(
                data.filter { it.tableGroupId in tableGroupId && it.tables.containsAll(tempDataTable) }
            )
